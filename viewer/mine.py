...
if __name__ == '__main__':
    ...
    uv2xy = hl2ss_3dcv.compute_uv2xy(calibration_ht.intrinsics, hl2ss.Parameters_RM_DEPTH_AHAT.WIDTH,
    ...
    producer.configure_rm_vlc(True, host, hl2ss.StreamPort.RM_VLC_LEFTFRONT, hl2ss.ChunkSize.RM_VLC,
                              hl2ss.StreamMode.MODE_1, hl2ss.VideoProfile.H264_MAIN, 1 * 1024 * 1024)
    producer.configure_rm_depth_ahat(True, host, hl2ss.StreamPort.RM_DEPTH_AHAT, hl2ss.ChunkSize.RM_DEPTH_AHAT,
                                     hl2ss.StreamMode.MODE_1, hl2ss.VideoProfile.H264_MAIN, 8 * 1024 * 1024)
    ...
    while (enable):
        ...
        _, data_ht = sink_ht.get_nearest(data_pv.timestamp)
        ...
        rgb = cv2.remap(data_lf.payload, calibration_lf.undistort_map[:, :, 0], calibration_lf.undistort_map[:, :, 1],
                        cv2.INTER_LINEAR)
        ht_to_lf_image = hl2ss_3dcv.camera_to_rignode(calibration_ht.extrinsics) @ hl2ss_3dcv.reference_to_world(
            data_ht.pose) @ hl2ss_3dcv.world_to_reference(data_lf.pose) @ hl2ss_3dcv.rignode_to_camera(
            calibration_lf.extrinsics) @ calibration_lf.intrinsics
